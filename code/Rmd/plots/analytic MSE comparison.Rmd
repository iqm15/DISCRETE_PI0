...
```{r}
mse_data <- data.frame(c(bias_pc**2 + estim_var_pc, bias_sto**2 + estim_var_sto, bias_hyb**2 + estim_var_hyb, bias_quad**2 + estim_var_quad, bias_quad_sto**2 + estim_var_quad_sto))
names(mse_data)[1] <- "mse"
mse_data["Estimator"] <- rep(c("PC", "STOREY", "PC_STO", "QUAD", "QUAD_STO"), each = length(mu1_grid))
mse_data["mu1"] <- rep(mu1_grid, 5)
# mse_data
...
plot_mse_mu <- ggplot(data = mse_data, aes(x=mu1, y=mse, group=Estimator, linetype=Estimator)) +
            geom_line() +
            scale_linetype(limits = c("STOREY", "PC", "PC_STO", "QUAD_STO"), labels = unname(TeX(c("$\\hat{m}_0^{Storey}$", "$\\hat{m}_0^{PC}$", "$\\hat{m}_0^{Poly}(1, 1/2)$", "$\\hat{m}_0^{Poly}(2, 1/2)$")))) +
            labs(x = TeX("$\\mu$"), y = "MSE") + 
...
```{r}
mse_data_2 <- data.frame(c(bias_pc**2 + estim_var_pc, bias_sto**2 + estim_var_sto, bias_hyb**2 + estim_var_hyb, bias_quad**2 + estim_var_quad, bias_quad_sto**2 + estim_var_quad_sto))
names(mse_data_2)[1] <- "mse"
mse_data_2["Estimator"] <- rep(c("PC", "STOREY", "PC_STO", "QUAD", "QUAD_STO"), each = length(m0_grid))
mse_data_2["pi0"] <- rep(pi0_grid, 5)
mse_data_2
...
```{r}
plot_name = "../../../xp_plot/mse/msegaussian.png"
